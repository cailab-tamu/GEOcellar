function SQLiteDatabaseBrowser14()
    ...
    function createUI()
        ...
        function connectDatabase(~, ~)
            try
                ...
                % Try different connection methods based on available functions
                try
                    % Method 1: Direct sqlite function (newer versions)
                    conn = sqlite(dbFile, 'readonly');
                catch
                    try
                        % Method 2: Using database function with SQLite driver
                        conn = database('', '', '', 'org.sqlite.JDBC', ...
                                       ['jdbc:sqlite:' dbFile]);
                    catch
                        % Method 3: Using databaseConnectionOptions (R2019b+)
                        opts = databaseConnectionOptions('sqlite', dbFile);
                        conn = database(opts);
                    end
                end
                
                % Test connection
                if isa(conn, 'database.sqlite.connection') || isa(conn, 'database.jdbc.connection') || isopen(conn)
        ...
        function loadTables(~, ~)
            try
                ...
                % Get list of tables - try different query methods
